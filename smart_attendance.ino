...
void handleRoot() {
  ...
  s += "<h1 class='text-3xl font-black text-white uppercase mb-8 border-b border-gray-800 pb-4'>CPI Smart Attendance Dashboard</h1>";
  
  s += "<div class='grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8'>";
  s += "<div class='lg:col-span-2 glass rounded-xl p-3'><iframe src='http://192.168.68.107:81/stream' class='w-full h-[350px] rounded-lg bg-black'></iframe></div>";
  s += "<div class='space-y-4'>";
  s += "<div class='glass p-6 rounded-xl border-l-4 border-blue-500'><p class='text-gray-500 text-xs uppercase'>Current User</p><h2 class='text-xl font-bold'>" + lastUser + "</h2></div>";
  ...
  s += "<h3 class='text-white font-bold mb-4 italic'>// ATTENDANCE LOGS</h3>";
  s += "<div class='overflow-x-auto'><table class='w-full text-left text-sm'>";
  s += "<thead class='text-gray-500 border-b border-gray-800'><tr>";
  s += "<th class='pb-3'>NAME</th><th class='pb-3'>ID</th><th class='pb-3'>DEPT</th><th class='pb-3 text-center'>DRESS</th><th class='pb-3 text-right'>TIME</th></tr></thead>";
  s += "<tbody class='text-gray-300'>" + attendanceLog + "</tbody></table>";
  if(attendanceLog == "") s += "<p class='text-center text-gray-600 py-6'>No scans detected yet.</p>";
  s += "</div></div></div>";
...
void setup() {
  ...
  drawUI("CPI SYSTEM", "SYSTEM READY", "WAITING FOR CARD");
}
  ...
  if (nfc.readPassiveTargetID(PN532_MIFARE_ISO14443A, uid, &uidLength, 50)) {
    for (int i = 0; i < 5; i++) {
      if (memcmp(uid, students[i].uid, 4) == 0) {
        ...
        String row = "<tr class='border-b border-gray-800'><td class='py-3 font-bold text-white'>" + students[i].name + "</td>";
        ...
        row += "<td class='py-3 text-right text-gray-500 font-mono'>SCAN_ACTIVE</td></tr>";
        attendanceLog = row + attendanceLog; 

        drawUI("SCAN COMPLETE", lastUser, lastStatus, true);
        delay(3000);
        drawUI("CPI SYSTEM", "READY", "WAITING...");
        break;
      }
    }
  }
